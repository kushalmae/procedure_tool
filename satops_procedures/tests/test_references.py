...
class SubsystemModelTest(TestCase):
    ...
    def test_ordering(self):
        Subsystem.objects.create(name="Power", mission=self.mission)
        Subsystem.objects.create(name="ADCS", mission=self.mission)
...
class ReferenceListViewTest(TestCase):
    ...
    def test_list_loads(self):
        response = self.client.get(
            reverse("reference_list", kwargs={"mission_slug": "test"})
        )
    ...
    def test_list_with_filters(self):
        sub = Subsystem.objects.create(name="Power", mission=self.mission)
        response = self.client.get(
            reverse("reference_list", kwargs={"mission_slug": "test"}),
            {
                "subsystem": sub.id,
...
class ReferenceDetailViewTest(TestCase):
    ...
    def test_detail_loads(self):
        ...
        response = self.client.get(
            reverse(
                "reference_detail",
                kwargs={"mission_slug": "test", "entry_id": entry.id},
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "ADCS ICD")
    ...
    def test_detail_404(self):
        response = self.client.get(
            reverse(
                "reference_detail",
                kwargs={"mission_slug": "test", "entry_id": 999},
            )
        )
...
class ReferenceCreateViewTest(TestCase):
    ...
    def test_create_requires_login(self):
        response = self.client.get(
            reverse("reference_create", kwargs={"mission_slug": "test"})
        )
        self.assertEqual(response.status_code, 302)
...
class ReferenceEditViewTest(TestCase):
    ...
    def test_edit_requires_login(self):
        response = self.client.get(
            reverse(
                "reference_edit",
                kwargs={"mission_slug": "test", "entry_id": self.entry.id},
            )
        )
        self.assertEqual(response.status_code, 302)
...
class CSVExportViewTest(TestCase):
    ...
    def test_export_csv(self):
        ...
        response = self.client.get(
            reverse("reference_csv_export", kwargs={"mission_slug": "test"})
        )
        self.assertEqual(response.status_code, 200)
...
class CSVImportViewTest(TestCase):
    ...
    def test_import_requires_login(self):
        response = self.client.post(
            reverse("reference_csv_import", kwargs={"mission_slug": "test"})
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn("login", response.url)
