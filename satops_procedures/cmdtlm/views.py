...
def command_list(request, mission_slug):
    if request.GET.get('clear'):
        if 'cmdtlm_cmd_filters' in request.session:
            del request.session['cmdtlm_cmd_filters']
        return redirect('cmdtlm_command_list', mission_slug=mission_slug)
    ...
    commands = _mission_filter(commands, request)
    if subsystem:
    ...
    subsystems_qs = _mission_filter(
        CommandDefinition.objects.values_list('subsystem', flat=True),
        request,
    )
    subsystems = (
        subsystems_qs
    ...
    categories_qs = _mission_filter(
        CommandDefinition.objects.values_list('category', flat=True),
        request,
    )
    categories = (
        categories_qs
...
def command_detail(request, mission_slug, command_id):
    qs = _mission_filter(CommandDefinition.objects.all(), request)
    command = get_object_or_404(qs, pk=command_id)
...
def telemetry_list(request, mission_slug):
    if request.GET.get('clear'):
        if 'cmdtlm_tlm_filters' in request.session:
            del request.session['cmdtlm_tlm_filters']
        return redirect('cmdtlm_telemetry_list', mission_slug=mission_slug)
    ...
    telemetry = _mission_filter(telemetry, request)
    if subsystem:
    ...
    subsystems_qs = _mission_filter(
        TelemetryDefinition.objects.values_list('subsystem', flat=True),
        request,
    )
    subsystems = (
        subsystems_qs
    ...
    data_types_qs = _mission_filter(
        TelemetryDefinition.objects.values_list('data_type', flat=True),
        request,
    )
    data_types = (
        data_types_qs
...
def telemetry_detail(request, mission_slug, telemetry_id):
    qs = _mission_filter(TelemetryDefinition.objects.all(), request)
    tlm = get_object_or_404(qs, pk=telemetry_id)
    enums = tlm.enums.all()
...
def csv_import(request, mission_slug):
    if request.method == 'POST':
        ...
        if not uploaded:
            ...
            return redirect('cmdtlm_csv_import', mission_slug=mission_slug)
...
def _import_commands(rows, request):
    count = 0
...
def _import_command_inputs(rows, request):
    count = 0
    mission = getattr(request, 'mission', None)
    cmd_qs = CommandDefinition.objects.all()
    if mission:
        cmd_qs = cmd_qs.filter(mission=mission)
    else:
        cmd_qs = cmd_qs.filter(mission__isnull=True)
...
def _import_telemetry(rows, request):
    count = 0
    mission = getattr(request, 'mission', None)
    for row in rows:
...
def csv_export_commands(request, mission_slug):
...
def csv_export_telemetry(request, mission_slug):
...
def _import_telemetry_enums(rows, request):
    count = 0
    mission = getattr(request, 'mission', None)
    tlm_qs = TelemetryDefinition.objects.all()
    if mission:
        tlm_qs = tlm_qs.filter(mission=mission)
    else:
        tlm_qs = tlm_qs.filter(mission__isnull=True)
    ...
    for row in rows:
        ...
        tlm = None
        if mnemonic:
            tlm = tlm_qs.filter(mnemonic=mnemonic).first()
        if not tlm and tlm_name:
            tlm = tlm_qs.filter(name=tlm_name).first()
        if not tlm:
            continue
